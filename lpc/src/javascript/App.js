Ext.define('CustomApp', {
    ...
    _findAlignedReleases: function() {
        ...
        this._aligned_release_oids.push(this_release_oid);
    ...
    _checkReleaseAlignment: function(release) {
        var parent_release = this._release_combo_box.getRecord();
        if (parent_release) {
    ...
    _findCurrentIteration: function() {
        ...
        var current_release = this._release_combo_box.getRecord();
        // if this is a past release, we want the 'current' iteration to be the last one
        if ( today_date > current_release.get('ReleaseDate')){
            me._current_iteration = me._iterations[me._iterations.length-1];
        } else {
            var counter = 0;
            var current_index = -1;
    
            Ext.Array.each(me._iterations,function(iteration){
                if ( iteration.get('StartDate') < today_date && iteration.get('EndDate') > today_date ) {
                    current_index = counter;
                    me._current_iteration_index = counter;
                    me._current_iteration = iteration;
                }
                counter++;
            });
        }
    ...
    _findAlignedIterations: function() {
        ...
        // Run the same query as _findIterationsForRelease(), but with projectScopeDown = true

        // dates are given in JS, but we need them to be ISO
        // Rally.util.DateTime.toIsoString(date, true); will return a date in UTC
        var start_date_iso = Rally.util.DateTime.toIsoString(this._release_combo_box.getRecord().get('ReleaseStartDate'), true);
    ...
    _findAcceptedItemsInEachIteration: function() {
        ...
        var iteration_query = [
            ...
            { property: "Release.Name", operator: "=", value: this._release_combo_box.getRecord().get("Name") }
        ];
    ...
    _findTodaysReleaseBacklog: function() {
        var me = this;
        var this_release = this._release_combo_box.getRecord().get('ObjectID');
    ...
    _assembleSprintData: function(){
        ...
        var worst_historical_actual_velocity = 0;
    ...
    _determineBestHistoricalActualVelocity: function(velocity_hash) {
        ...
        for ( var i in velocity_hash ) {
            if (velocity_hash[i] > 0 && counter !== this._current_iteration_index ) {
